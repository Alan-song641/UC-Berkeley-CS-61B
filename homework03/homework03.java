...
public class homework03 {
    ...
    public static void smoosh(int[] ints) {
        ...
        for (int i = 0; i < ints.length - 1; i++) {   //你妈嗨 这里写了i<13没看出来 应该是它的长度-1才对
            if (ints[i] == ints[i + 1]) {
                outints[k] = -1;
                k--;
            } else {
        ...
        outints[m] = ints[ints.length - 1];   //到最底下的时候，没有将两者不同的后者放到outints里面，这里要加上
    ...
    public static void main(String[] args) {
        ...
        for (i = 0; i < test6.length; i++) {
            list6.insertEnd(new Integer(test6[i]));  //将某一个数组放入自己构造的Slist中的处理方法
        }
