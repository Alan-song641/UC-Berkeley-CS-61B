...
public class UDGraph
{
    ...
    public UDGraph paths(int length) {
        UDGraph newGraph = new UDGraph(vertices);
        // Put your answer to Part II here.  【BFS】
            ...
            if(length<=1){
                ...
                for(int i=0;i<vertices;i++)
                    for(int j=0;j<vertices;j++){
                        ...
                        //利用recursion
                        //利用的精华在于：public UDGraph paths(int length)的返回值是UDGraph itself
                        if(paths(length-1).hasEdge(i, j)){
                            //它的return值 是i和j有edge的话，edge的长度为length-1
                            for(int k=0;k<vertices;k++){
                                if(hasEdge(j,k)){   //如果j和k有edge的话，i和k的长度就为（length-1）+1
                                    newGraph.addEdge(i, k);
                                }
                            }
                        }
